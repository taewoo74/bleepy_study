...
interface InsightViewType {
  ...
  tableData: any;
  settingPopup: (str: string, str2: string, str3: string, str4: string, bol: boolean) => void
  tabState: string;
}



const InsightView = ({ visitorData, chartData, tableData, settingPopup, tabState }: InsightViewType) => {
          ...
          <div className="mt-4 flex">
            <div className="num_container flex-row flex">
              <div className="font-semibold text-xs text-gray-400 mt-3 ml-3 mr-auto flex direction-row relative" >
                방문횟수
                <img onMouseOver={() => setTooltip(true)} onMouseLeave={() => setTooltip(false)} className='w-[13px] h-[13px] flex mt-0.5 ml-0.5' src={icon} />
                {tooltip && (
            ...
            <div className="num_container flex-row flex">
              <div className="font-semibold text-xs text-gray-400 mt-3 ml-3 mr-auto">일일활성사용자</div>
              <div className='mt-14 mr-4' >{visitorData.dau}</div>
            </div>
            <div className="num_container flex-row flex">
              <div className="font-semibold text-xs text-gray-400 mt-3 ml-3 mr-auto">신규 방문자 수</div>
              <div className='mt-14 mr-4' >{visitorData.visitor}</div>
            </div>
            <div className="num_container flex-row flex">
              <div className="font-semibold text-xs text-gray-400 mt-3 ml-3 mr-auto">재 방문자 수</div>
      ...
      <div className='mt-10  h-[300px]' >
        ...
        {!!tableData && (
          <div className='mt-8 w-f' >
            <div onClick={onClickExcel} className='flex bg-[#EEEEEE] w-[120px] h-[31px] text-[12px] py-1.5 px-4 ml-auto mb-2 rounded' >  <img className='w-[16px] h-[16px] mt-0.5 mr-[3px]' src={xlsx} /> 엑셀 다운로드</div>
