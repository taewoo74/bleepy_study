...
const Insight = () => {
    ...
    const [visitorData, SetVisitorData] = useState<visitorType>({ visitorTotal: 0, dau: 0, visitor: 0, returnVisitor: 0 })
    ...
    const getChartData = async (data: { startDate: string, endDate: string }) => {
        ...
        chartData.forEach((arr: any) => {
            ...
            one.dau = arr.dau;
            one.newVisitorCount = arr.newVisitorCount;
            one.returningVisitorCount = arr.returningVisitorCount;
            chartLine.push(one);
        });

        const LineArr = [{ dataKey: 'visite', color: '#FF3D00', name: '방문횟수' }, { dataKey: 'dau', color: '#FF7A30', name: '일일활성사용자(DAU)' },
        { dataKey: 'newVisitorCount', color: '#FCB25C', name: '신규 방문자 수' }, { dataKey: 'returningVisitorCount', color: '#FFECDE', name: '재 방문자 수' }]
    ...
    return (
        <div className='flex flex-col w-[1220px] pt-10 px-10' >
            <InsightHeader onChangeStartDate={onChangeStartDate} DateSetting={DateSetting} submitInsightData={submitInsightData} onChangeEndDate={onChangeEndDate} startDate={startDate} endDate={endDate} tabState={tabState} onClickTabMenu={onClickTabMenu} dateState={dateState} datePickerFormat={datePickerFormat} />
            <InsightView visitorData={visitorData} chartData={chartData} tableData={tableData} settingPopup={settingPopup} tabState={tabState}   />
        </div>
    )
}
